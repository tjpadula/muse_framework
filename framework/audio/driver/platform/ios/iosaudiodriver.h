...
public:
    ...
    bool open(const Spec& spec, Spec* _Nonnull activeSpec) override;
    void close() override;
...
//    std::vector<unsigned int> availableOutputDeviceSampleRates() const override;
    ...
    std::vector<samples_t> availableOutputDeviceBufferSizes() const override;       // new
    std::vector<sample_rate_t> availableOutputDeviceSampleRates() const override;   // different returnn type
