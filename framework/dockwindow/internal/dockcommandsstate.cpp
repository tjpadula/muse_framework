...
std::string DockCommandsState::moduleName() const
{
    return "dock";
}

void DockCommandsState::init()
{
...
void DockCommandsState::deinit()
{
}

void DockCommandsState::updateCommandStates(const std::vector<Command>& commands)
{
    IF_ASSERT_FAILED(m_moduleRegister) {
        return;
    }
...
CommandState DockCommandsState::commandState(const Command& command) const
{
    UNUSED(command);
    return CommandState(true, false);
}

async::Channel<Command, CommandState> DockCommandsState::commandStateChanged() const
{
