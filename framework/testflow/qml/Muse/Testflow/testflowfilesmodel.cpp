...
TestflowFilesModel::TestflowFilesModel(QObject* parent)
    : QAbstractListModel(parent), Contextable(muse::iocCtxForQmlObject(this))
{
}

void TestflowFilesModel::init()
{
    //m_fileIndex = testflow()->currentFileIndex();
    ...
    //    testflow()->fileFinished().onReceive(this, [this](const File& f) {
    //        updateFile(f);
    //    });
...
QVariant TestflowFilesModel::data(const QModelIndex& index, int role) const
{
...
int TestflowFilesModel::rowCount(const QModelIndex&) const
{
    return static_cast<int>(m_files.val.size());
}

QHash<int, QByteArray> TestflowFilesModel::roleNames() const
{
...
void TestflowFilesModel::update()
{
    beginResetModel();

//    m_files = testflow()->files();
...
void TestflowFilesModel::updateFile(const File& f)
{
...
QString TestflowFilesModel::fileStatus(const File& f) const
{
